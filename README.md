...
[![Actions
Status](https://github.com/sims1253/hrvester/workflows/Tests/badge.svg)](https://github.com/sims1253/hrvester/actions)
[![Codecov test
...
`hrvester` is an R package designed for analyzing Heart Rate Variability
(HRV) data from `.fit` files generated by Garmin devices (and
potentially others). It focuses on orthostatic tests (measuring HRV
while laying down and then standing up) to provide insights into an
athlete’s recovery and readiness for training.

The package implements research-validated preprocessing methods
including: - **Kubios-style artifact detection** with time-varying
thresholds - **Multiple correction methods**: Linear interpolation,
cubic spline, and Lipponen-Tarvainen algorithms - **Comprehensive
quality assessment** following clinical HRV standards - **Automated
...
- **Research-Grade Preprocessing Pipeline:** Implements state-of-the-art
  artifact correction methods validated in peer-reviewed literature
  - **Kubios-style Detection:** Time-varying thresholds with dRR series
    analysis for robust artifact identification
  - **Multiple Correction Methods:** Linear interpolation (lowest RMSSD
    bias), cubic spline (Kubios-compatible), and Lipponen-Tarvainen (\<
    2% error)
  - **Quality Assessment:** Clinical-grade quality metrics with
    A/B/C/D/F grading system
- **Orthostatic Test Processing:** Full pipeline from raw `.fit` files
  to readiness scores, including:
  - **RR Interval Preprocessing:** Robust artifact detection,
    physiological plausibility checks, and research-validated correction
    methods
  - **HRV Metric Calculation:** Calculates key HRV metrics like RMSSD
    and SDNN with quality-assured data
  - **Resting Heart Rate Estimation:** Uses a robust algorithm to
    determine resting heart rate
  - **Neural Recovery Score:** Proprietary algorithm for athlete
    readiness assessment
  - **Trend Analysis:** Moving averages and trend detection for
    long-term monitoring
- **Data Management:**
  - **Intelligent Caching:** Automatic caching system with quality
    metrics storage to avoid reprocessing
  - **Quality Filtering:** Configurable quality thresholds to
    automatically exclude poor measurements
  - **Batch Processing:** Process entire directories of `.fit` files
    efficiently
- **Visualization & Reporting:**
  - **Interactive Dashboards:** Comprehensive HRV trend visualization
    with ggplot2
  - **Training Recommendations:** Data-driven insights based on HRV
    trends and readiness scores
...
The package offers multiple research-validated artifact correction
methods:
...
The preprocessing pipeline implements methods validated in peer-reviewed
literature:

- **Linear Interpolation**: Consistently shows lowest RMSSD bias (Giles
  et al., 2018)
- **Cubic Spline**: Kubios-compatible method balancing automation with
  accuracy (Singh et al., 2020)  
- **Lipponen-Tarvainen**: State-of-the-art algorithm achieving \<2% HRV
