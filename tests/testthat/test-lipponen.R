...
test_that("Simple missed beat is detected (long interval)", {
  missed_beat_data <- tibble::tibble(time = c(rep(800, 5), 1600, rep(800, 5))) # One long interval
  result <- classify_hrv_artefacts_lipponen(missed_beat_data)
  expect_equal(result$classification[6], "long") # should be "long", *not* "missed" (at this stage)

  # Check that subsequent beat isn't misclassified due to being a neighbor
  expect_equal(result$classification[7], "normal")
})

# Test case 5: Simple extra beat (short interval)
test_that("Simple extra beat is detected (short interval)", {
  extra_beat_data <- tibble::tibble(time = c(rep(800, 5), 400, 1200, rep(800, 5))) # Short, then long
...
test_that("Ectopic beat is detected (PNP pattern)", {
  set.seed(2)
    ectopic_data <- tibble::tibble(time = c(rnorm(100, 1000, 25), 1400, 400, rnorm(100, 1000, 25))) # Long, then short
...
test_that("Ectopic beat is detected (NPN pattern)", {
    ectopic_data2 <- tibble::tibble(time = c(rep(1000, 5), 400, 1400, rep(1000, 5)))  #Short, then long
...
test_that("Split long beat is detected", {
  split_long_data <- tibble::tibble(time = c(rep(800, 5), 1200, 1300, rep(800, 5)))  # two "long"
  result <- classify_hrv_artefacts_lipponen(split_long_data)
    expect_equal(result$classification[6], "long")
    expect_equal(result$classification[7], "long")
})

# Test case 9:  Short beat, followed by another short (split short)
test_that("Split short beat is detected", {
  split_short_data <- tibble::tibble(time = c(rep(800, 5), 600, 500, rep(800, 5)))  # two "short"
  result <- classify_hrv_artefacts_lipponen(split_short_data)
  expect_equal(result$classification[6], "short")
   expect_equal(result$classification[7], "short")
})

# Test case 10:  Mixed artefacts
test_that("Mixed artefacts are detected correctly", {
  mixed_data <- tibble::tibble(time = c(rep(800, 5), 1600, 400, 800, 950, 650, 800, 500, 1100)) # long, short, ectopic (PNP), short
  result <- classify_hrv_artefacts_lipponen(mixed_data)
    expect_equal(result$classification[6], "long")
    expect_equal(result$classification[7], "short")
    expect_equal(result$classification[9], "ectopic")
    expect_equal(result$classification[12], "short")
})
...
test_that("Trailing short beat is detected correctly", {
    ...
    expect_equal(result$classification[11], "short")
})
...
test_that("long beat classified as missed", {
    long_missed <- tibble::tibble(time = c(rep(800, 5), 1600, rep(800, 5)))
    result <- classify_hrv_artefacts_lipponen(long_missed)
    expect_equal(result$classification[6], "long") #Not yet corrected
})

#Test case 14: Check if short beat is classified as extra if it meets the extra condition
test_that("short beat classified as missed", {
    short_extra <- tibble::tibble(time = c(rep(800, 5), 400, 400, rep(800, 5)))
    result <- classify_hrv_artefacts_lipponen(short_extra)
    expect_equal(result$classification[6], "short") #Not yet corrected
    expect_equal(result$classification[7], "short") #Not yet corrected
})
...
test_that("c1 and c2 parameters work correctly",{
    ...
    result_high_c <- classify_hrv_artefacts_lipponen(ectopic_data, c1=10, c2=10)
